...
function fmtWeekRange(monday: Date): string {
  ...
  return `${monday.toLocaleDateString('en-IN', opts)} – ${sunday.toLocaleDateString('en-IN', { ...opts, year: 'numeric' })}`;
}
...
const TeamMemberCard = ({
  ...
  const totalActivities = (plan.planData || []).reduce((s, d) => s + d.activities.length, 0);

  const startEdit = () => {
    setEditDays(JSON.parse(JSON.stringify(plan.planData || [])));
    setEditing(true);
    setExpanded(true);
  };
  ...
  const displayDays = editing ? editDays : (plan.planData || []);
...
export const WeeklyPlanScreen = ({ navigation }: any) => {
  ...
  const loadMyPlan = useCallback(async () => {
    ...
    try {
      ...
      if (plan && plan.id) {
        setMyPlan(plan);
        setPlanDays(plan.planData || buildEmptyWeek(weekMonday));
      } else {
        setMyPlan(null);
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      if (myPlan?.id) {
        await weeklyPlanApi.update(myPlan.id, planDays);
      } else {
