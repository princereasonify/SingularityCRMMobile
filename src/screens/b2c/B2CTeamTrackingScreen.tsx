...
// Normalize the /team-live payload (nested latestPing) into a flat shape for the UI.
...
export const B2CTeamTrackingScreen = () => {
  ...
  const load = useCallback((initial = false) => {
    ...
    b2cTrackingService.getTeamLive()
  ...
  const jumpToTeamMember = (id: string) => {
    setPickerOpen(false);
    const m = team.find(t => String(t.id) === id);
    if (!m) return;
    setSelected({ id: Number(id), name: m.name, role: m.role || 'Agent', distance: 0 });
  };

  const teamOptions = useMemo(
    () => team.map(m => ({ label: m.name || `Member #${m.id}`, value: String(m.id) })),
    [team],
  );
        ...
        {team.length > 0 && (
          <View style={{ zIndex: 20 }}>
            ...
            <Trigger
              ...
              icon={<Users size={15} color={pickerOpen ? T.accent : T.dim} strokeWidth={2} />}
            />
            {pickerOpen && (
              <Dropdown
                options={teamOptions}
                onSelect={jumpToTeamMember}
                style={{ width: '100%' }}
                maxHeight={280}
              />
            )}
